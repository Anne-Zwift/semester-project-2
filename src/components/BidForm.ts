...
export function BidForm(item:Listing): HTMLElement {
  ...
  container.className = 'my-6 p-6 bg-gray-50 rounded-2xl border border-gray-100';
  ...
  if (isOwner) {
    ...
    ownerMsg.className = 'text-sm text-warning font-semibold italic text-center';
  ...
  if (isExpired) {
    ...
    endedMsg.textContent = 'This auction has ended. No more bids can be placed.';
...
function renderActiveForm(container: HTMLElement, item: Listing) {
  ...
  input.className = 'p-3 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-navy';
